...
신규 회차 또는 잔여 수가 바뀐 회차는 공개 좌석표에서 일반석·장애인석·좌석 열을 모두 확인한 뒤에만 알림을 보냅니다. CGV 좌석 상세 조회가 일시적으로 실패하거나 정보가 불완전하면 알림을 보내지 않고 다음 1분 주기에 다시 확인합니다. 따라서 장애인석이나 A열만 남은 회차가 판별 실패 때문에 잘못 알림되는 일을 막습니다.
