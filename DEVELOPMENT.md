...
`SCAN_MODE=full`(기본)은 매 주기 감시 기간 28일을 모두 조회합니다. `CGV_REQUEST_SPACING_SECONDS=2` 때문에 한 주기에 최소 약 56초가 걸리고, 좌석 상세 조회가 붙으면 `POLL_INTERVAL_SECONDS`를 넘겨 사실상 쉬는 시간 없이 돌아갑니다.

`SCAN_MODE=cursor`는 조회 범위를 좁힙니다. 상태 파일에 **프론티어** — IMAX 회차가 관측된 가장 늦은 상영일 — 를 저장하고, 매 주기 다음만 조회합니다.

```
[오늘 .. 프론티어]        이미 열린 구간 (잔여 좌석 변화 추적용)
...
```text
조회 범위: 전체 모드, 28일 요청 (2026-08-13~2026-09-09)
조회 범위: 커서 모드, 17일 요청 (2026-08-13~2026-08-29)
```

`full`로 운영하며 로그를 확인한 뒤, Railway 환경변수에 `SCAN_MODE=cursor`를 넣어 전환하세요. 문제가 생기면 다시 `full`로 되돌리면 즉시 원래 동작으로 돌아갑니다.
