...
오늘·내일은 매 주기, 그 뒤 `+2`~`+6`은 5주기에 한 번씩 돌아옵니다. 주기당 3일치가 늘어나므로 하루 5회차 기준 좌석 상세 요청이 15건, `CGV_REQUEST_SPACING_SECONDS=2` 기준 주기가 약 30초 길어집니다. `+7` 이후 날짜는 잔여 수가 바뀔 때만 좌석표를 읽습니다. 세 값 중 앞의 두 개를 `0`으로 두면 이 기능이 꺼지고 이전 동작(잔여 수 변화에만 반응)으로 돌아갑니다.
