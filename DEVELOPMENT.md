...
구독자마다 chat_id, 표시 이름, 채팅 유형, 구독일, 설정 네 가지가 나옵니다. Telegram이 4096자를 넘는 메시지를 거부하므로 목록이 길어지면 들어가는 만큼만 싣고 `… 외 N명`으로 끊습니다. 글자 수 기준이라 이름이 긴 그룹이 섞여 있어도 메시지가 통째로 실패하지 않습니다.
