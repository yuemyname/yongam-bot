...
`HTTP 429`는 CGV가 좌석 상세 요청을 일시적으로 제한한 상태입니다. 이때 전체 잔여 좌석이 6석 이하면 알림을 보류하고 다음 주기에 다시 조회하며, 7석 이상이면 좌석 종류를 확인하지 못했다는 표시와 함께 전체 잔여 수 기준으로 알림을 보냅니다. 봇은 신규 회차 또는 잔여 수가 실제로 바뀐 회차만 좌석표를 요청해 불필요한 제한도 줄입니다. 제한이 계속되면 `POLL_INTERVAL_SECONDS=120`으로 늘릴 수 있습니다.
