...
구독자 알림은 기본적으로 16개 작업자가 동시에 전송하며 초당 25건으로 제한합니다. 이는 Telegram 무료 봇의 약 30건/초 대량 전송 한도 아래에서 신규 오픈 알림을 빠르게 전달하기 위한 값입니다. 필요하면 `TELEGRAM_BROADCAST_WORKERS`와 `TELEGRAM_BROADCAST_RATE_PER_SECOND`로 조정할 수 있지만, 초당 전송 수를 30보다 높게 설정하지 마세요.

구독자 목록과 설정, 처리한 Telegram 명령 위치, 기존 예매 알림 기록과 좌석 수·A열 제외 좌석 위치, 마지막 좌석 알림 시각, 실패한 일정 재조회 목록은 `/data/notified.json`에 저장됩니다. Railway가 재시작되거나 새 버전을 배포해도 유지됩니다.
...
2026-08-14 00:55:11 KST INFO 전송: 예매 가능 좌석 • 2026-08-16 23:00 — IMAX관 / 좌석 9/624석 (9석), 성공 3명, 실패 0명
2026-08-14 00:55:11 KST INFO 전송: 예매 가능 좌석 • 2026-08-16 23:30 — IMAX관 / 좌석 8/624석 (8석) ⚠️A열 여부 미확인, 성공 3명, 실패 0명
2026-08-14 00:55:12 KST INFO 조회 완료: 성공 15일, 오류 0일, IMAX 회차 24개, 신규 2개, 예매 가능 좌석 1개, ...
```

**`전송:` 으로 시작하는 줄만 실제로 메시지가 나간 것입니다.** 나머지 `제외:`·`보류:` 줄은 판정 결과이고, 회차 전체 상태는 `--verbose`의 `회차 판정` 줄에 있습니다. 좌석표에서 A열 여부를 읽지 못한 채 내보낸 알림은 전송 줄 끝에 `⚠️A열 여부 미확인`이 붙습니다.
