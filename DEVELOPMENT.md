...
1. 7061234567 · 홍길동 · 개인 · 08-19
   신규 오픈 + 잔여 좌석 · 모든 A열 제외 좌석 · 1석부터 모두
2. -1001234567890 · 용아맥 대기방 · 그룹 · 08-16
   신규 오픈만 · 모든 A열 제외 좌석 · 1석부터 모두
```

설정한 적 없는 구독자는 기본값(`신규 오픈 + 잔여 좌석`, `모든 A열 제외 좌석`, `1석부터 모두`)으로 집계됩니다.

구독자마다 chat_id, 표시 이름, 채팅 유형, 구독일, 설정 세 가지가 나옵니다. Telegram이 4096자를 넘는 메시지를 거부하므로 목록이 길어지면 들어가는 만큼만 싣고 `… 외 N명`으로 끊습니다. 글자 수 기준이라 이름이 긴 그룹이 섞여 있어도 메시지가 통째로 실패하지 않습니다.
