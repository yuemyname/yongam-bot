...
`HTTP 429`는 CGV가 좌석 상세 요청을 일시적으로 제한한 상태입니다. 이때 해당 회차 알림은 보류되며 다음 주기에 다시 조회합니다. 봇은 신규 회차 또는 잔여 수가 실제로 바뀐 회차만 좌석표를 요청해 불필요한 제한도 줄입니다. 제한이 계속되면 `POLL_INTERVAL_SECONDS=120`으로 늘릴 수 있습니다.
