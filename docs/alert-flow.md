...
`/count_2`도 잔여 좌석 변경 알림에만 적용되며, **한 번의 확인에서 늘어난 좌석 수**를 봅니다. 좌석이 팔려서 줄어든 변화는 늘어난 수가 0이므로 전달되지 않고, 1매씩 두 번에 나눠 취소된 경우도 각 시점에서는 1매뿐이라 전달되지 않습니다. `/seat_sweet`과 함께 쓰면 명당 구역 안에서 늘어난 수로 판단합니다. 좌석표에서 A열 여부를 읽지 못했을 때는 전체 잔여 수의 증가분으로 판단합니다 — 알림 본문이 기준으로 삼는 값과 같습니다.
