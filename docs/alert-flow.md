...
flowchart TD
    ...
    F --> G[예매 마감 회차 제외]
    G --> H[좌석 상세 조회<br/>좌석 수가 바뀌었거나<br/>재확인 차례인 회차]
    H --> I[오픈·좌석 알림 발송]
    I --> J[상태 파일 저장]
    J --> D
...
발송이 결정돼도 구독자 전원에게 가지는 않습니다. 구독자는 `/mode`로 **알림 종류**를, `/seat`으로 **좌석 대상**을, `/count`로 **최소 취소 매수**를 고릅니다. 세 설정은 서로 독립적으로 작동합니다.
...
| 💺 잔여 좌석 | 받음 | — | 받음 | 명당 좌석이 남았을 때만 | 2석 이상 늘었을 때만 |
| ⚠️ 조회 오류 공지 | 받음 | 받음 | 받음 | 영향 없음 | 영향 없음 |

`/seat_sweet`은 잔여 좌석 변경 알림에만 적용됩니다. 정확한 행·번호를 확인한 뒤 F16~29, G16~29, H13~32, I13~32, J11~34, K11~34, L11~34 중 현재 남은 좌석이 바뀐 경우에만 전송합니다. 좌석표에서 A열 여부를 읽지 못하면 명당 여부를 판별할 수 없어 `/seat_sweet` 구독자에게는 보내지 않습니다. 예매 오픈 알림은 항상 전송됩니다.

`/count_2`도 잔여 좌석 변경 알림에만 적용되며, **한 번의 확인에서 늘어난 A열 제외 좌석 수**를 봅니다. 좌석이 팔려서 줄어든 변화는 늘어난 수가 0이므로 전달되지 않고, 1매씩 두 번에 나눠 취소된 경우도 각 시점에서는 1매뿐이라 전달되지 않습니다. `/seat_sweet`과 함께 쓰면 명당 구역 안에서 늘어난 수로 판단합니다.

전체 잔여 수는 기준으로 쓰지 않습니다. 전체 수에는 A열이 섞여 있어서, 2석이 풀렸어도 그중 하나가 A열이면 `/count_2`가 요구한 2매가 아니기 때문입니다. 그래서 좌석표에서 A열 여부를 읽지 못한 회차(`⚠️ A열 여부 미확인`)는 확인된 A열 제외 좌석 수만으로 판단하며, 그 수조차 읽지 못하면 `/count_2` 구독자에게는 보내지 않습니다. 좌석표를 못 읽는 회차의 진짜 2매 취소를 놓칠 수 있다는 뜻이고, 이는 `/seat_sweet`이 같은 상황에서 취하는 정책과 동일합니다.

기본값(`/count_1`)은 문턱이 1이라는 뜻이 아니라 **문턱이 없다**는 뜻입니다. 좌석이 팔려서 줄어든 변화도 그대로 받습니다.
