...
발송이 결정돼도 구독자 전원에게 가지는 않습니다. 구독자는 `/mode`로 **알림 종류**를, `/seat`으로 **좌석 대상**을 고릅니다. 두 설정은 서로 독립적으로 작동합니다.
...
| 알림 | `/mode_all` | `/mode_open` | `/mode_seats` | `/seat_sweet` 적용 시 |
|---|:---:|:---:|:---:|---|
| 🎟️ 예매 오픈 | 받음 | 받음 | — | 영향 없음 |
| 💺 잔여 좌석 | 받음 | — | 받음 | 명당 좌석이 남았을 때만 |
...
**설정한 적 없는 구독자는 전부 받습니다.** 저장된 값이 없으면 `/mode_all` + `/seat_all`로 취급합니다.
