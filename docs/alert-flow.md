...
발송이 결정돼도 구독자 전원에게 가지는 않습니다. 구독자는 `/mode`로 **알림 종류**를, `/day`로 **상영일**, `/seat`으로 **좌석 대상**을, `/count`로 **예매 가능 최소 좌석**을 고릅니다. 네 설정은 서로 독립적으로 작동합니다.
...
**설정한 적 없는 구독자는 전부 받습니다.** 저장된 값이 없으면 `/mode_all` + `/day_all` + `/seat_all` + `/count_1`로 취급합니다.
