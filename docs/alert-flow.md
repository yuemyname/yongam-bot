...
봇은 조회 → 판정 → 발송을 약 2분 주기로 반복합니다. **신규 오픈 후보를 맨 먼저 확인한 뒤, 날짜 하나를 읽으면 그 자리에서 좌석 상세까지 처리하고 알림을 보냅니다.** 취소표는 시간이 지나면 값이 사라지므로 다음 날짜로 넘어가기 전에 발송합니다.
...
HTTP 403도 첫 응답에서 남은 조회를 멈춥니다. 일반 실행은 120초 후 재시도하지만, 운영자가 `CGV_RECOVERY_REQUEST_ID`로 단일 재확인을 요청하면 **1시간 중단 → 요청 1건 확인 → 성공 시 정상 조회 / 실패 시 자동 조회 중단**으로 동작합니다. 재확인 실패 후에는 재배포해도 추가 요청하지 않으며, 복귀 후 다시 403을 만나도 중단합니다. Telegram 명령과 기존 발송 대기열은 계속 처리합니다.

이미 발송한 날짜의 알림은 그대로 남습니다. 실패했거나 403·429 뒤에 생략된 날짜는 상태 파일에 저장해 다음 정상 주기에 신규 오픈 후보 다음 순서로 우선 재조회합니다.

조회 실패 공지는 `.env`의 `TELEGRAM_CHAT_ID`, 즉 **운영자에게만** 갑니다. 구독자가 할 수 있는 일이 없고 봇이 알아서 재조회하기 때문입니다. 운영자가 `/stop`으로 구독을 해지했어도 이 공지는 받습니다.

---

## 2. 회차 하나가 통과해야 하는 관문

**처음 보는 상영일·시각이면 관문을 건너뜁니다.** 매진이든 A열만 남았든 예매 오픈 알림이 바로 나갑니다. 아래 관문은 이미 알린 회차의 좌석 알림에만 적용되고, 대부분은 여기서 조용히 탈락합니다.
...
발송이 결정돼도 구독자 전원에게 가지는 않습니다. 구독자는 `/mode`로 **알림 종류**를, `/day`로 **상영 요일**, `/date`로 **잔여좌석 상영 날짜**, `/time`으로 **잔여좌석 상영 시간**, `/seat`으로 **좌석 대상**을, `/count`로 **예매 가능 최소 좌석**을 고릅니다. 여섯 설정은 함께 적용됩니다.

```mermaid
flowchart TD
    A[발송이 결정된 알림] --> B{알림 종류}
    B -->|조회 오류 공지| Z["운영자 1명<br/>구독자에게는 안 감"]
...
| 알림 | `/mode_all` | `/mode_open` | `/mode_seats` | `/day_weekend` 적용 시 | `/seat_sweet` 적용 시 | `/count_2` 적용 시 |
|---|:---:|:---:|:---:|---|---|---|
| 🎟️ 예매 오픈 | 받음 | 받음 | — | 토·일 상영분만 | 영향 없음 | 영향 없음 |
| 💺 예매 가능 좌석 | 받음 | — | 받음 | 토·일 상영분만 | 명당 좌석이 남았을 때만 | 2석 이상 남았을 때만 |
| ⚠️ 조회 오류 공지 | — | — | — | 운영자에게만 | 운영자에게만 | 운영자에게만 |

`/day_weekend`은 **알림을 받은 날이 아니라 영화 상영일**이 토요일 또는 일요일인지 판단합니다. 평일에 주말 회차가 새로 열려도 즉시 받으며, 공휴일은 별도로 주말로 취급하지 않습니다. 운영 공지에는 적용하지 않습니다.
...
`/time 18:00 23:59`는 한국시간 18:00~23:59 시작 회차의 잔여좌석 알림만 허용합니다. 양 끝 시각을 포함하고 `/time 22:00 02:00`처럼 자정을 넘는 범위도 지원합니다. **신규 오픈·공지에는 적용하지 않습니다.** 요일 기준과 CGV 조회 대상은 그대로 유지하며, 대기열 재시도도 최신 시간 설정을 확인합니다.

`/date 20261003`은 10월 3일 포함 이후, `/date 20261003 20261005`는 10월 3~5일의 잔여좌석 알림만 허용합니다. 기준은 CGV 상영일입니다. **신규 오픈·공지에는 날짜 범위를 적용하지 않으며**, 기존 요일 설정은 유지됩니다. 날짜·시간 등 조건은 함께 적용되고, 대기열 재시도도 최신 설정을 확인합니다. 실제 CGV 조회 범위·주기는 바꾸지 않습니다. 기간이 지나도 날짜 조건은 자동 해제되지 않습니다.

**설정한 적 없는 구독자는 전부 받습니다.** 저장된 값이 없으면 `/mode_all` + `/day_all` + `/date_all` + `/time_all` + `/seat_all` + `/count_1`로 취급합니다.
...
| 환경변수 | 기본값 | 바꾸면 생기는 일 |
|---|---:|---|
| `POLL_INTERVAL_SECONDS` | `120` | 조회 주기. 줄이면 감지가 빨라지지만 CGV 차단 위험이 커집니다. |
