...
프론티어 바로 다음의 신규 오픈 후보 `+1일 → +2일 → +3일`을 먼저 조회하고, 이어서 `오늘 → 내일 → … → 프론티어` 순서로 이미 열린 날짜를 조회합니다. 신규 오픈 탐색이 항상 주기의 맨 앞에서 끝나므로, 그 뒤로는 날짜마다 좌석 상세까지 끝내고 다음 날짜로 넘어가도 오픈 감지가 늦어지지 않습니다.
