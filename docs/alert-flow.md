...
발송이 결정돼도 구독자 전원에게 가지는 않습니다. 알림은 종류가 나뉘고, 구독자는 `/mode`, `/seat`, `/seats`로 받을 종류와 좌석을 고릅니다. 세 설정은 서로 독립적으로 작동합니다.
...
`/seats sweet`도 잔여 좌석 변경 알림에만 적용됩니다. 정확한 행·번호를 확인한 뒤 F16~29, G16~29, H13~32, I13~32, J11~34, K11~34, L11~34 중 현재 남은 좌석이 바뀐 경우에만 전송합니다. 예매 오픈 알림은 항상 전송됩니다.

**설정한 적 없는 구독자는 전부 받습니다.** 저장된 값이 없으면 `/mode_all` + `/seat_all` + `/seats all`로 취급하므로, 기능이 추가되기 전부터 구독하던 사람의 수신 내용은 달라지지 않았습니다.
